...
public class Employee {
	...
	public static void main(String[] args) {
		...
		// 2. 남자 사원의 평균 연령 구하기
		int sumAge =0;		// 나이합 변수
		int mcount = 0;		// 남자사원수 변수 
		double mavgAge = 0; // 남자사원 평균 나이
		char c = 'M';		// 성별 남자
		
		for (int i = 0; i < gender.length; i++) {
			if(gender[i] == c)	{	//문자 == 가능	//성별 남자
			//if(gender.equals(c)) {		//성별 남자
				sumAge += age[i];	    //나이 누적
				mcount = mcount +1;		//인원수 누적
				//count++;
				}
			
		}
		mavgAge =(double)sumAge / mcount;	// 남자 나이 평균
		System.out.printf("2. 남자 사원의 숫자는 : %d명 이며 평균 연령은 %.1f 세 입니다.",mcount,mavgAge);
		System.out.println();
		
		// 3. 여성 사원들의 평균 급여 구하기
		int sumPay =0;	// 급여합 변수
		int fcount = 0; // 여자 사원수
		double favgPay = 0.0;	// 여자사원 평균 급여
		char f = 'F';		//성별 여성
		...
		favgPay = (double)sumPay / fcount;
		System.out.printf("3. 여자 사원의 숫자는 : %d명 이며 평균 급여는 %.1f만원 입니다.",fcount,favgPay);
		System.out.println();
		
		//4. 일본에 근무하는 사원들의 이름과 연락처 구하기		이름 이어붙이기, split으로 자르기
...
//			System.out.println(phoneString);
		...
		// 이름, 연락처 분리
		String[] arrName = nameString.split(",");		// 이름 분리해서 이름 배열에 해당
		String[] arrPhone = phoneString.split(",");		// 연락처 분리해서 연락처 배열에 할당
		
		System.out.println("4.일본사원들의 이름과 연락처");
